...
(
    SELECT 
      time::DATE AS date, 
      SUM (CASE WHEN status LIKE '%404%' THEN 1 ELSE 0 END)::NUMERIC / COUNT (status) * 100 AS percentage 
    FROM log 
    GROUP BY date
)
AS errors
...
if __name__ == '__main__':
    ...
    print_result(msg1, articles, show_article=1, show_popularity=1)
    authors = get_result_set(query2)  # loading top authors
    print_result(msg2, authors, show_author=1, show_popularity=1)
